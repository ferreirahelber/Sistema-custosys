...
export default function App() {
  // Adicionei 'dashboard' como tipo possível
  const [view, setView] = useState<'dashboard' | 'settings' | 'ingredients' | 'recipes' | 'costs'>('settings');
  ...
  useEffect(() => {
    ...
    if (settings.labor_monthly_cost > 0 && settings.work_hours_monthly > 0) {
      ...
      // Se já configurado, vai para Dashboard, senão fica em Settings
      setView('dashboard'); 
    }
  }, []);
  ...
  const handleSettingsSaved = () => {
    setIsConfigured(true);
    setView('dashboard'); // Redireciona para Dashboard após configurar
  };
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50">
      ...
      <aside className="w-full md:w-64 bg-white border-r border-slate-200 p-6 flex flex-col h-screen sticky top-0">
        ...
        {isConfigured && (
            <div className="mt-auto px-2">
                <p className="text-xs text-slate-400 text-center">Versão 1.2.0 • Pro</p>
            </div>
        )}
      </aside>
      ...
      <main className="flex-1 p-6 md:p-12 overflow-y-auto h-screen">
        {/* Header Dinâmico (Removido daqui se for dashboard, pois o dashboard tem o seu próprio) */}
        {view !== 'dashboard' && (
