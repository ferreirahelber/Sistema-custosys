...
create table if not exists recipe_items (
  ...
  -- Tipo do Item: 'ingredient' ou 'recipe' (Sub-receita)
  item_type text not null default 'ingredient',
  
  -- Referências (apenas uma das duas deve ser preenchida conforme item_type)
...
-- ==========================================
-- 6. AUTOMAÇÃO DE PREÇOS (TRIGGER)
-- ==========================================

-- Função para recalcular custos de receitas quando um ingrediente muda de preço
...
begin
  ...
  if (old.unit_cost_base is distinct from new.unit_cost_base) then
    ...
    loop
      ...
      -- Busca a receita para atualizar custos fixos/trabalho se necessário (simplificado aqui para focar no material)
      -- Nota: Para uma consistência total, idealmente recalculariamos tudo, mas o foco é o material.
      -- Assumindo que o trigger de update na receita cuidaria do resto se fosse complexo, 
      -- mas aqui faremos um update direto nos campos de custo.
      
      select * into r_recipe from recipes where id = r_item.recipe_id;
      
      v_old_cost := r_recipe.unit_cost;
      
      -- Recálculo Simples (Mantendo Trabalho e Overhead constantes por enquanto, já que mudou só material)
      -- Total Final = Material + Labor + Overhead
      ...
      set 
        ...
        unit_cost = (v_total_material + total_cost_labor + total_cost_overhead) -- Assumindo rendimento já fatorado ou unitário direto
        -- Nota: Se unit_cost depende do rendimento, ajustar: / nullif(yield_units, 0)
      where id = r_item.recipe_id;

      -- Opcional: Registrar histórico se a mudança for grande
...
-- Tabela de Histórico de Preços (Se não existir, criar para suportar a função acima)
