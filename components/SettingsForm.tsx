...
export const SettingsForm: React.FC<Props> = ({ onSave }) => {
  ...
  const applyCalculatedRate = () => {
      ...
      toast.info(`Taxa de ${calculatedRate.toFixed(2)}% aplicada! Salve as configurações para confirmar.`);
  };
            ...
            {costMode === 'detailed' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
                    <div className="space-y-4">
                        ...
                        <div className="flex justify-between p-3 bg-slate-100 rounded-lg font-bold text-slate-800 text-sm"><span>Total de Despesas:</span><span>R$ {totalFixedExpenses.toFixed(2)}</span></div>
                    </div>
