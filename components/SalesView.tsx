...
import { TrendingUp, Plus, Trash2, Calendar, Tag } from 'lucide-react';
...
export function SalesView() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await FinancialService.addSale({
  ...
  async function handleDelete(id: string) {
    // Em vez de window.confirm, usamos o toast customizado
    toast('Deseja excluir esta receita?', {
      action: {
        label: 'Excluir',
        onClick: async () => {
          try {
            await FinancialService.deleteSale(id);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
          <Plus className="w-5 h-5 text-emerald-600" /> Nova Receita
        </h3>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="Ex: Venda de Bolo de Cenoura"
          ...
          <div>
            <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-medium py-2 px-4 rounded-lg transition flex items-center justify-center gap-2">
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr><td colSpan={5} className="p-8 text-center text-slate-500">Nenhuma receita registrada.</td></tr>
            ) : (
              items.map(item => (
